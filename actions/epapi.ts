...
export async function epApiQuery(event) {
    const baseUrl = 'https://equipmentgram.com/wp-json/gf/v2';
    ...
    if (!formId) {
        ...
        url = `${baseUrl}/${parseInt(formId)}/entries?sorting[key]=id&sorting[direction]=DESC&sorting[is_numeric]=true&paging[page_size]=${parseInt(pageSize || 20)}&paging[current_page]=${parseInt(currentPage || 0)}`;                
    }
    console.log(url);
